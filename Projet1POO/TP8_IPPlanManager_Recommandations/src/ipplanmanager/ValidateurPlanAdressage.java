/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/Classes/Class.java to edit this template
 */
package ipplanmanager;

import java.util.ArrayList;

public class ValidateurPlanAdressage {

    public void verifierAdresses(ArrayList<ResultatVLSM> res)
            throws AdresseIPInvalideException {

        for (ResultatVLSM r : res) {
            CalculateurReseau.verifierAdresseIP(r.getAdresseReseau());
        }
    }

    public void verifierChevauchements(ArrayList<ResultatVLSM> res)
            throws ChevauchementReseauException {

        for (int i = 0; i < res.size(); i++) {
            for (int j = i + 1; j < res.size(); j++) {

                ResultatVLSM r1 = res.get(i);
                ResultatVLSM r2 = res.get(j);

                if (CalculateurReseau.reseauxSeChevauchent(
                        r1.getAdresseReseau(), r1.getCidr(),
                        r2.getAdresseReseau(), r2.getCidr())) {

                    throw new ChevauchementReseauException(
                            "Chevauchement : " +
                            r1.getNomBesoin() + " / " +
                            r2.getNomBesoin()
                    );
                }
            }
        }
    }

    public void afficherValidationReussie() {
        System.out.println("Validation OK : aucun conflit détecté.");
    }
}